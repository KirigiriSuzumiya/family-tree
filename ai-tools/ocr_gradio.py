...
chat_client = Qianfan(
    access_key=auth['chat_api_key'],
    secret_key=auth['chat_secret_key'],
)
...
def chat_submit(msg, history):
    return "", history + [[msg, None]]

def chat_fun(history):
    ...
    for stream in completion:
        for c in stream.choices:
            if c.delta.content:
                history[-1][-1] += c.delta.content
        yield history


def chat_fun_knowledge(history, resource_info, conversation_id):
    if len(history) == 1:
        conversation_id = knowledge_chat.get_conversation_id()
    history[-1][-1] = ""
    if not resource_info:
        resource_info = ""
    file_names = []
    for chunk in knowledge_chat.run_conversation(conversation_id, history[-1][0]):
        if chunk.get("answer"):
            history[-1][-1] += chunk.get("answer")
        if chunk.get("content"):
            for content in chunk.get("content"):
                if content.get("event_type") == "RAGAgent" and content.get("outputs").get("name_cn")=="知识问答":
                    for hit_doc in content.get("outputs").get("references"):
                        if hit_doc.get("title") not in file_names:
                            file_names.append(hit_doc.get("title"))
                            file_content = hit_doc.get("content")
                            resource_info +=f"""<details> <summary>{hit_doc.get("title")}</summary>
                                                {file_content}
                                                </details>
                                            """
        print(resource_info)
        yield history, resource_info, conversation_id
...
with gr.Blocks(title="Face+OCR+Chat demo") as demo:
    ...
    with gr.Tab("ChatBot"):
        with gr.Tab("Knowledge Base"):
            with gr.Column():
                ...
                with gr.Accordion("Image Caption Embedding"):
                    with gr.Row():
                        input_image_bot = gr.Image(sources="upload", type="filepath")
                ...
                with gr.Row():
                    chatbot_knowledge = gr.Chatbot(height=800, scale=2,
                                                   label="LLM chatbot", placeholder="say something and try it out!")
                    with gr.Accordion("Knowledge Source"):
                        knowledge_source = gr.HTML(label="Knowledge Source", show_label=True)
                msg_knowledge = gr.Textbox(label="input your words", placeholder="push 'ENTER' to submit")
                msg_knowledge.submit(chat_submit,
                                     [msg_knowledge,chatbot_knowledge],
                                     [msg_knowledge,chatbot_knowledge]).then(
                                        chat_fun_knowledge, 
                                        [chatbot_knowledge, knowledge_source, conversation_id],
                                        [chatbot_knowledge, knowledge_source, conversation_id])
        ...
        with gr.Tab("Normal Chat"):
            with gr.Column():
                gr.Markdown("Notice: This module is under development and may be unstable,please report any error you meet to make our service better.")
                chatbot = gr.Chatbot(height=800, label="LLM chatbot", placeholder="say something and try it out!")
                msg = gr.Textbox(label="input your words", info="push 'ENTER' to submit")
                msg.submit(chat_submit, [msg,chatbot], [msg, chatbot]).then(chat_fun, chatbot, chatbot)
                clr_btn = gr.Button("Clear")
                clr_btn.click(lambda: None, None, chatbot)
